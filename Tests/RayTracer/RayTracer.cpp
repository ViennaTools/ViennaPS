...
int main()
{
    ...
    auto dom = lsSmartPointer<lsDomain<NumericType, D>>::New(bounds, boundaryCons, gridDelta);
    ...
    {
        auto trench = lsSmartPointer<lsDomain<NumericType, D>>::New(bounds, boundaryCons, gridDelta);
        ...
        lsBooleanOperation<NumericType, D>(dom, trench, lsBooleanOperationEnum::RELATIVE_COMPLEMENT).apply();
    }
         ...
         time += advectionKernel.getAdvectedTime())
    {
        ...
        auto velocities = lsSmartPointer<velocityField>::New(std::move(mcestimates));
        advectionKernel.setVelocityField(velocities);
        advectionKernel.apply();

        // auto meshAdvect = lsSmartPointer<lsMesh<NumericType>>::New();
        // lsToSurfaceMesh<NumericType, D>(newLayer, meshAdvect).apply();
        // lsVTKWriter<NumericType>(meshAdvect, "trench-" + std::to_string(counter) + ".vtk")
        //     .apply();
