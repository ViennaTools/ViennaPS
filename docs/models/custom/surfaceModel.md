...
- Coverages and fluxes are stored as `viennacore::PointData`.
...
```c++
  void initializeCoverages(unsigned numSurfacePoints) override {
    ...
    coverages = ps::SmartPointer<viennacore::PointData<NumericType>>::New();
    coverages->insertNextScalarData(someCoverage, "someCoverage");
  }
```
To use coverages, it is essential to initialize the class member `coverages` with a new instance of `viennacore::PointData`. If the `coverages` variable is left as `nullptr`, no coverages will be utilized during the simulation. To initialize a single coverage, a container with a size equal to the number of surface points must be created and inserted into the `viennacore::PointData`. Additionally, a name for the coverage can be specified during initialization. This designated name should then be used in `updateCoverages()` or `calculateVelocities()` to access the specific coverage as needed.
...
```c++
  void updateCoverages(ps::SmartPointer<viennacore::PointData<NumericType>> particleFluxes,
...
public:
  ...
  ps::SmartPointer<std::vector<NumericType>> calculateVelocities(
      ps::SmartPointer<viennacore::PointData<NumericType>> rates,
