...
vps.psMakeTrench(psDomain=geometry, gridDelta=params['gridDelta'], 
                 xExtent=params['xExtent'], yExtent=params['yExtent'],
                 trenchWidth=params['trenchWidth'], trenchDepth=params['trenchHeight'],
                 taperingAngle=params['taperAngle'], baseHeight=0., periodicBoundary=False,
                 makeMask=False, material=vps.psMaterial.Si).apply()

# copy top layer to capture deposition
geometry.duplicateTopLevelSet(vps.psMaterial.SiO2)

# process model encompasses surface model and particle types
model = vps.TEOSDeposition(stickingProbabilityP1=params['stickingProbabilityP1'],
