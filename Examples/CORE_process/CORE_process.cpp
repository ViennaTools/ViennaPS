...
int main()
{
    ...
    volumeMesh.setFileName("initial");
    volumeMesh.apply();
    ...
    // auto peParticle = std::make_unique<EtchantPoly<NumericType, D>>();
    ...
    removeProcessModel->setSurfaceModel(clearModel);
    ...
    {   
        ...
        // oxidize
        oxidizeProcess.setProcessDuration(0.8);
        oxidizeProcess.apply();
        volumeMesh.setFileName("Mesh_" + std::to_string(counter++));
        volumeMesh.apply();
        std::cout << "Oxidize done" << std::endl;

        // remove
        removeProcess.setProcessDuration(1);
        removeProcess.apply();
        volumeMesh.setFileName("Mesh_" + std::to_string(counter++));
        volumeMesh.apply();
        ...
        // etch
        etchProcess.setProcessDuration(5);
        etchProcess.apply();
        volumeMesh.setFileName("Mesh_" + std::to_string(counter++));
        volumeMesh.apply();
        std::cout << "Etch done" << std::endl;
