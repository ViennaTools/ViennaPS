...
class VelocityField : public psVelocityField<NumericType> {
  ...
  const std::vector<psMaterial> &epitaxyMaterials;
...
public:
  VelocityField(const std::array<NumericType, 3> passedDir100,
                ...
                const std::vector<psMaterial> &passedEpitaxyMaterials)
    ...
    directions[1] = rayInternal::Diff(
        directions[1], rayInternal::Scale(rayInternal::DotProduct(
  ...
  getScalarVelocity(const std::array<NumericType, 3> & /*coordinate*/,
    ...
    for (auto epitaxyMaterial : epitaxyMaterials) {
      if (psMaterialMap::isMaterial(material, epitaxyMaterial)) {
...
public:
  psSelectiveEpitaxy(const std::vector<psMaterial> pEpitaxyMaterials)
...
private:
  ...
  std::vector<psMaterial> epitaxyMaterials;
};