 ...
 * 
 * This class handles reading a Process Simulation Domain (psDomain) from a binary
 * file previously created with psWriter.
 */
...
public:
  ...
  Reader(std::string passedFileName)
      : fileName(std::move(passedFileName)) {}

  Reader(SmartPointer<Domain<NumericType, D>> passedDomain, std::string passedFileName)
  ...
  SmartPointer<Domain<NumericType, D>> apply() {
    ...
    if (std::string(identifier).compare(0, 8, "psDomain")) {
      Logger::getInstance()
          .addWarning("Reading domain from stream failed. Header could not be found.")
          .print();
      return domain;
    }
    ...
    if (formatVersion > 0) { // Update this when version changes
      Logger::getInstance()
          .addWarning("Reading domain of version " + std::to_string(formatVersion) +
    ...
    if (hasCellSet) {
      ...
      Logger::getInstance()
          .addWarning("CellSet deserialization not yet implemented in psReader.")
          .print();
    }
