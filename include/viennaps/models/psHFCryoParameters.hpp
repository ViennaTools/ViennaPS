...
template <typename NumericType> struct HFCryoParameters {
  ...
  struct ModelConfigType {
    bool useTemperatureDependence =
        true;                     // Frenkel-Arrhenius/Arrhenius for rates
    ...
    NumericType T_ref =
        300.; // fixed reference T when !useTemperatureDependence
  } Config;

  // Surface diffusion of physisorbed HF along the surface
  // D_s(T) = D0 * exp(-omega * E_des / kB*T)
  // E_diff = omega * E_des  (corrugation ratio from Lill 2023, Table IV:
  // omega~0.2-0.3)
  ...
  NumericType k_r_direct() const {
    return DirectReaction.A_r *
           std::exp(-DirectReaction.E_a / (constants::kB * temperature));
  }
  ...
  NumericType effective_k_des() const {
    const NumericType T =
        Config.useTemperatureDependence ? temperature : Config.T_ref;
  ...
  NumericType effective_k_r_direct() const {
    const NumericType T =
        Config.useTemperatureDependence ? temperature : Config.T_ref;
    return DirectReaction.A_r *
           std::exp(-DirectReaction.E_a / (constants::kB * T));
  }
};
