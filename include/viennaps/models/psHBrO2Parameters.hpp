...
template <typename NumericType> struct HBrO2Parameters {
  // fluxes in (1e15 /cm² /s)
  ...
  std::unordered_map<int, NumericType> beta_Br = {{1, 0.1}, {0, 0.1}};
  ...
  struct SiType {
    ...
    NumericType theta_g_ie =
        constants::degToRad(86); // angle where yield is zero [rad]
