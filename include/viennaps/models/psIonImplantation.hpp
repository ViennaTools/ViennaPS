...
class IonImplantation : public ProcessModelBase<NumericType, D> {
  ...
  class ImplantCallback : public AdvectionCallback<NumericType, D> {
    ...
    bool useEmbeddedBoundaries_ = false;

  public:
    viennacs::Implant<NumericType, D> &implant() { return implant_; }
    void setUseEmbeddedBoundaries(bool enable) {
      useEmbeddedBoundaries_ = enable;
    }
...
public:
  ...
  // When true, rebuild the cell set with embedded boundary points enabled
  // before implanting. This enables tilt-corrected sub-grid surface offsets
  // via ray-plane intersection. If the domain was already built with
  // withEmbeddedBoundaries=true, calling this is not necessary but harmless.
  void enableEmbeddedBoundaries(bool enable = true) {
    callback_->setUseEmbeddedBoundaries(enable);
  }
};
