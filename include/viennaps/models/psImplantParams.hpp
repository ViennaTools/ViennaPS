...
template <typename NumericType, int D> struct ImplantParams {
...
inline void applyImplantParams(IonImplantation<NumericType, D> &implant,
                               const ImplantTableParams<NumericType> &p) {
  implant.setImplantModel(
      SmartPointer<ImplantRecipeModel<NumericType, D>>::New(p.implantRecipe));
  implant.setDamageModel(
      SmartPointer<DamageRecipeModel<NumericType, D>>::New(p.damageRecipe));
}
