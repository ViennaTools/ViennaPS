...
class HBrO2SurfaceModel : public SurfaceModel<NumericType> {
public:
  using SurfaceModel<NumericType>::coverages;
  using SurfaceModel<NumericType>::surfaceData;
  const HBrO2Parameters<NumericType> &params;

  HBrO2SurfaceModel(const HBrO2Parameters<NumericType> &pParams)
      : params(pParams) {}
  ...
  void updateCoverages(SmartPointer<viennals::PointData<NumericType>> fluxes,
    ...
    for (size_t i = 0; i < numPoints; ++i) {
      auto Gb_Br = etchantFlux->at(i) * params.etchantFlux;
      ...
      auto a = (params.Si.k_sigma + 2 * GY_ie) / Gb_Br;
      auto b = (params.Si.beta_sigma + GY_o) / Gb_O;

      eCoverage->at(i) = Gb_Br < 1e-6 ? 0. : 1 / (1 + (a * (1 + 1 / b)));
...
template <typename NumericType, int D>
class HBrO2Ion
    : public viennaray::Particle<HBrO2Ion<NumericType, D>, NumericType> {
public:
  HBrO2Ion(const HBrO2Parameters<NumericType> &pParams)
      : params(pParams),
...
template <typename NumericType, int D>
class HBrO2Neutral
    : public viennaray::Particle<HBrO2Neutral<NumericType, D>, NumericType> {
  const HBrO2Parameters<NumericType> &params;
  const std::string fluxLabel;
  const std::unordered_map<int, NumericType> &beta_map;

public:
  HBrO2Neutral(const HBrO2Parameters<NumericType> &pParams,
  ...
  void surfaceCollision(NumericType rayWeight, const Vec3D<NumericType> &,
    ...
    if (params.fluxIncludeSticking) {
      const auto &phi_Br = globalData->getVectorData(0)[primID];
      const auto &phi_O = globalData->getVectorData(1)[primID];
      NumericType beta = sticking(materialId);
      S_eff = beta * std::max(1. - phi_Br - phi_O, 0.);
    }
  ...
  surfaceReflection(NumericType rayWeight, const Vec3D<NumericType> &rayDir,
    ...
    NumericType beta = sticking(materialId);
    NumericType S_eff = beta * std::max(1. - phi_Br - phi_O, 0.);
...
/// Model for etching Si in a HBr/O2 plasma. The model is based on the paper by
/// Belen et al., Vac. Sci. Technol. A 23 (5), 1430–1439 (2006),
/// DOI: https://doi.org/10.1116/1.2013317
template <typename NumericType, int D>
...
private:
  void initializeModel() {
    ...
    auto ion = std::make_unique<impl::HBrO2Ion<NumericType, D>>(params);
    this->insertNextParticleType(ion);
    auto etchant = std::make_unique<impl::HBrO2Neutral<NumericType, D>>(
        params, "etchantFlux", params.beta_Br);
    this->insertNextParticleType(etchant);
    if (params.oxygenFlux > 0) {
      auto oxygen = std::make_unique<impl::HBrO2Neutral<NumericType, D>>(
    ...
    // surface model
    auto surfModel =
        SmartPointer<impl::HBrO2SurfaceModel<NumericType, D>>::New(params);
