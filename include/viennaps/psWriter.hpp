 ...
 * 
 * This class handles serializing a Process Simulation Domain (psDomain) to a binary
 * file. The file format (.psst - Process Simulation State) contains all levelSets,
 * cell data, material mappings and domain setup information.
 */
...
public:
  ...
  Writer(SmartPointer<Domain<NumericType, D>> passedDomain, std::string passedFileName)
  ...
  void apply() {
    ...
    if (fileName.find(".psst") != fileName.length() - 5) {
      Logger::getInstance()
          .addWarning("File name does not end in '.psst', appending it.")
          .print();
      fileName.append(".psd");
    }
    ...
    if (hasMaterialMap) {
      ...
      fout.write(reinterpret_cast<const char*>(&numMaterials), sizeof(uint32_t));
