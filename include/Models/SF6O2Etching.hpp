...
template <class T>
class SF6O2VolumeIon : public csParticle<SF6O2VolumeIon<T>, T> {
public:
  SF6O2VolumeIon(const T passedMeanEnergy = 100.,
...
template <typename NumericType, int D>
class SF6O2VolumeModel : public psVolumeModel<NumericType, D> {
protected:
  using psVolumeModel<NumericType, D>::domain;
  using psVolumeModel<NumericType, D>::tracer;

public:
  SF6O2VolumeModel(const NumericType energy, const NumericType meanFreePath,
                   const int maskID) {
    tracer.setNumberOfRaysPerPoint(1000);
    tracer.setExcludeMaterialId(maskID + 1);

    auto damageIon =
        std::make_unique<SF6O2VolumeIon<NumericType>>(energy, meanFreePath);
...
public:
  SF6O2Etching(const double ionFlux, const double etchantFlux,
               ...
               const int maskMaterial = 0, const bool useVolumeModel = false) {
    processModel = psSmartPointer<psProcessModel<NumericType, D>>::New();

    // particles
    auto ion = std::make_unique<SF6O2Ion<NumericType, D>>(ionEnergy);
    ...
    if (useVolumeModel) {
      auto volumeModel = psSmartPointer<SF6O2VolumeModel<NumericType, D>>::New(
          ionEnergy, 1, maskMaterial);
