...
plasmaNeutralCollision(viennaray::gpu::PerRayData *prd, unsigned int primID) {
  atomicAdd(
      &launchParams
...
plasmaNeutralReflection(const void *sbtData, viennaray::gpu::PerRayData *prd,
  ...
  const auto &phi_E = coverages[primID];
  const auto &phi_P = coverages[primID + launchParams.numElements];
...
__forceinline__ __device__ void plasmaNeutralReflectionNoPassivation(
...
plasmaIonCollision(const void *sbtData, viennaray::gpu::PerRayData *prd,
  ...
  gpu::PlasmaEtchingParameters *params =
      reinterpret_cast<gpu::PlasmaEtchingParameters *>(launchParams.customData);
  ...
  if (static_cast<BuiltInMaterial>(material) == BuiltInMaterial::Polymer &&
    ...
    f_sp_theta =
        (params->Polymer_a1 * c + params->Polymer_a2 * c * c +
         params->Polymer_a3 * c * c * c + params->Polymer_a4 * c * c * c * c) /
        params->Polymer_aSum;
...
plasmaIonReflection(const void *sbtData, viennaray::gpu::PerRayData *prd,
                    unsigned int primID) {
  viennaps::gpu::PlasmaEtchingParameters *params =
      reinterpret_cast<viennaps::gpu::PlasmaEtchingParameters *>(
