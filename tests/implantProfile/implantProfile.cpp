...
void testPearsonIVProfile() {
  ...
  ps::ImplantPearsonIV<T, D> model(params, /*lateralMu=*/0.,
                                   /*lateralSigma=*/30.);
  ...
  const T expected =
      model.getDepthProfile(params.mu) * model.getLateralProfile(0., params.mu);
...
void testDualPearsonIVProfile() {
  ...
  ps::ImplantDualPearsonIV<T, D> model(
...
void testHoblerDamageProfile() {
  ...
  ps::ImplantDamageHobler<T, D> model(rp, sigma, lambda, defectsPerIon,
                                      lateralSigma);

  // Peak should be near rp
  VC_TEST_ASSERT(model.getDepthProfile(rp) >
                 model.getDepthProfile(rp + 5. * sigma));
...
void testImplantMaterialName() {
  ...
  VC_TEST_ASSERT(ps::implantMaterialName(viennaps::Material::BulkSi) ==
                 "silicon");
  VC_TEST_ASSERT(ps::implantMaterialName(viennaps::Material::aSi) == "silicon");
  VC_TEST_ASSERT(ps::implantMaterialName(viennaps::Material::PolySi) ==
                 "silicon");
  VC_TEST_ASSERT(ps::implantMaterialName(viennaps::Material::Ge) ==
                 "germanium");
  ...
  VC_TEST_ASSERT(ps::implantMaterialName(viennaps::Material::Si3N4) ==
                 "nitride");
  VC_TEST_ASSERT(ps::implantMaterialName(viennaps::Material::SiN) == "nitride");
  // Custom material: unknown
  VC_TEST_ASSERT(ps::implantMaterialName(viennaps::Material::custom(42)) ==
                 "unknown");
}
...
void testTableBackedModels() {
  ...
  ps::ImplantTableModel<T, D> implant(implantTable, "phosphorus", "silicon",
                                      "crystalline", 15., 7., 0., 1e13, 0., 0.);
  ps::DamageTableModel<T, D> damage(damageTable, "phosphorus", "silicon", 15.,
                                    7., 0., 1e13, 0.);
