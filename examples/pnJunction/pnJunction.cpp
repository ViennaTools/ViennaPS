...
SmartPointer<Domain<T, D>> buildSubstrate(T xExtent, T substrateDepth,
  ...
  T bounds[2 * D] = {-0.5 * xExtent, 0.5 * xExtent, -substrateDepth, domainTop};
...
// ────────────────────────────────
void runImplant(SmartPointer<Domain<T, D>> domain, const std::string &concLabel,
                const std::string &damageLabel, T rp, T sigma, T skewness,
                T kurtosis, T rpTail, T sigmaTail, T skewnessTail, T kurtTail,
                T headFraction, T dose, T tiltDeg, T lateralSigmaHead,
                T lateralSigmaTail) {
  ...
  head.gamma = kurtosis;
  tail.mu = rpTail;
  tail.sigma = sigmaTail;
  tail.beta = skewnessTail;
  tail.gamma = kurtTail;

  auto implantModel = SmartPointer<ImplantDualPearsonIV<T, D>>::New(
      head, tail, headFraction, T(0), lateralSigmaHead, T(0), lateralSigmaTail);
...
void writeDepthProfile(SmartPointer<Domain<T, D>> domain,
  ...
  if (!data) {
    std::cerr << "Field " << fieldLabel << " not found\n";
    return;
  }
...
void writeLateralProfile(SmartPointer<Domain<T, D>> domain,
                         const std::string &fieldLabel, T atDepth,
                         const std::string &filename) {
  auto cs = domain->getCellSet();
  const T delta = cs->getGridDelta();
  auto *data = cs->getScalarData(fieldLabel);
  ...
  for (int i = 0; i < cs->getNumberOfCells(); ++i) {
    ...
    if (depth < T(0))
      continue;
    if (std::abs(depth - atDepth) > delta * T(0.6))
      continue;
  ...
  for (const auto &[x, sv] : bins)
    f << std::fixed << std::setprecision(3) << x << "," << sv.first / sv.second
      << "\n";
  std::cout << "  Wrote " << filename << "\n";
}
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "  tilt=" << pTilt << "°  rot=" << pRotation << "°";
  std::cout << "  left half)\n";
  ...
  std::cout << "  tilt=" << bTilt << "°  rot=" << bRotation << "°";
  std::cout << "  right half)\n";
  ...
  std::cout << "  (Integrals span the whole domain; each side contributes "
               "~half.)\n\n";
  ...
  writeLateralProfile(domain, "net_doping", scanDepth,
