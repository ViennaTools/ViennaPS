...
def write_depth_profile(
    ...
    for idx in range(n):
        ...
        if depth not in bins:
            bins[depth] = {"count": 0, "sum": [0.0] * n_fields, "max": [0.0] * n_fields}
        b = bins[depth]
        b["count"] += 1
    ...
    with open(filename, "w") as out:
        ...
        out.write(
            "# depth_nm: positive distance into substrate"
...
def run(cfg_path: str) -> None:
    ...
    head.mu = cfg["projectedRange"]
    head.sigma = cfg["depthSigma"]
    head.beta = cfg["skewness"]  # → C++ params.beta (β₂ position)
    head.gamma = cfg["kurtosis"]  # → C++ params.gamma (γ₁ position)

    lateral_mu = cfg.get("lateralMu", 0.0)
    lateral_sigma = cfg.get("lateralSigma", head.sigma)

    head_fraction = cfg.get("headFraction", -1.0)
    if head_fraction > 0.0:
        tail = ps.PearsonIVParameters()
        tail.mu = cfg["tailProjectedRange"]
        tail.sigma = cfg["tailDepthSigma"]
        tail.beta = cfg["tailSkewness"]
        tail.gamma = cfg["tailKurtosis"]
        tail_mu = cfg.get("tailLateralMu", 0.0)
        tail_sigma = cfg.get("tailLateralSigma", lateral_sigma)
        ...
        description = (
            ...
            f"{cfg['energyKeV']:.0f} keV, "
            f"{cfg.get('angle',0.):.0f} deg tilt "
            f"(dual-Pearson IV, head fraction {head_fraction:.4f})"
        )
    else:
        implant_model = ps.ImplantPearsonIV(head, lateral_mu, lateral_sigma)
        description = (
            f"{species} into {cfg.get('material','Si')} at "
            f"{cfg['energyKeV']:.0f} keV "
            f"(single Pearson IV)"
    ...
    damage_model = ps.ImplantDamageHobler(
        cfg["damageProjectedRange"],
        cfg["damageVerticalSigma"],
        cfg["damageLambda"],
        cfg["damageDefectsPerIon"],
        cfg["damageLateralSigma"],
    ...
    implant.setDose(cfg.get("doseCm2"))
    implant.setTiltAngle(cfg.get("angle", 0.0))
    ...
    # ── Anneal model ──────────────────────────────────────────────────────────
    durations = cfg["annealStepDurations"]
    temperatures = cfg["annealTemperatures"]
    peak_T = max(temperatures)

    anneal = ps.Anneal()
    anneal.setTemperatureSchedule(durations, temperatures)
    anneal.setArrheniusParameters(cfg["annealD0"], cfg["annealEa"])
    anneal.setMode(ps.AnnealMode.GaussSeidel)
    anneal.setImplicitSolverOptions(
        int(cfg.get("annealImplicitMaxIterations", 400)),
        float(cfg.get("annealImplicitTolerance", 1e-6)),
        float(cfg.get("annealImplicitRelaxation", 1.0)),
    )
    ...
    if int(cfg.get("annealDefectCoupling", 1)):
        ...
        anneal.setDefectDiffusivities(
            cfg.get("annealInterstitialDiffusivity", 0.0),
            cfg.get("annealVacancyDiffusivity", 0.0),
        )
        anneal.setDefectReactionRates(
            cfg.get("annealRecombinationRate", 0.0),
            cfg.get("annealInterstitialSinkRate", 0.0),
            cfg.get("annealVacancySinkRate", 0.0),
        )
        si = cfg.get("annealScoreIFactor", 0.5)
        sv = cfg.get("annealScoreVFactor", 0.5)
        if si + sv > 0:
    ...
    if int(cfg.get("annealSolidActivation", 1)):
        ...
        anneal.setSolidSolubilityArrhenius(
            cfg.get("annealSolidSolubilityC0"),
            cfg.get("annealSolidSolubilityEa"),
        )
    ...
    if "annealInterstitialEqC0" in cfg:
        ...
        anneal.setDefectEquilibriumArrhenius(
            cfg["annealInterstitialEqC0"],
            cfg["annealInterstitialEqEa"],
            cfg["annealVacancyEqC0"],
            cfg["annealVacancyEqEa"],
        )
    ...
    if "annealClusterKfc" in cfg or "annealClusterKr" in cfg:
        ...
        anneal.setDefectClusterKinetics(
            cfg.get("annealClusterKfi", 0.0),
            cfg.get("annealClusterKfc", 0.0),
            cfg.get("annealClusterKr", 0.0),
        )
        anneal.setDefectClusterInitFraction(cfg.get("annealClusterInitFraction", 0.01))
    ...
    write_depth_profile(domain, [label_total, label_damage], "profile_post_implant.csv")

    ps.Process(domain, anneal, 0.0).apply()
    domain.getCellSet().writeVTU("post_anneal.vtu")
    write_depth_profile(
        domain,
        [label_total, label_active, label_damage, label_interstitial, label_vacancy],
